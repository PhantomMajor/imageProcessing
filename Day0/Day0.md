...
Here, 
    Mat is an object.
    Img is the name of the image (identifier)
    n_row is the number of rows
    N_col is the number of columns
    CV_8UC3 is a keyword containing the format type of the image
        CV is the openCV library.
...
Class is pass-by-reference, struct is pass-by-value.
Class can create subclass which can inherit parent’s properties, struct cannot.
Informally, Class can contain functions and methods, but struct cannot.


In openCV, the colors are saved as BGR channel values.
...
* To compile, put Makefile in the directory, go to the working directory in the terminal, write “make FILENAME” to compile, write “./FILENAME” to run.
